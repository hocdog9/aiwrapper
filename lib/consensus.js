...
function extractClaims(content) {
  ...
  const mathBlocks = [];
  const protectedText = cleaned.replace(/(\${1,2})(.+?)\1/g, (match) => {
    const token = `__MATH_BLOCK_${mathBlocks.length}__`;
    mathBlocks.push(match);
    return token;
  });

  const restoreMath = (value) => value.replace(/__MATH_BLOCK_(\d+)__/g, (_, index) => mathBlocks[Number(index)] || '');

  const fragments = protectedText
    .split(/[.!?]+|\n+/)
    .map((part) => restoreMath(part)
      .replace(/(^|\s)#{1,6}\s*/g, '$1')
      .replace(/\*\*(.*?)\*\*/g, '$1')
      .replace(/__(.*?)__/g, '$1')
      .replace(/^[\s>*-]+/, '')
